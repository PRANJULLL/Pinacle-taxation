...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btnViewTransactionComplete').addEventListener('click', () => {
        if (activeViewedTask) {
            taskViewModal.hide();
            setTimeout(() => {
                transactionCompleteTask(activeViewedTask);
            }, 300);
        }
    });
        ...
        if (activeViewedTask) {
            ...
            setTimeout(() => {
                if (activeViewedTask.status === 'Transaction Completed') {
                    revertToCompletedTask(activeViewedTask);
                } else {
    ...
    async function transactionCompleteTask(task) {
        try {
            const res = await fetch(`api/tasks.php?id=${task.id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
    ...
    async function revertToCompletedTask(task) {
        const result = await Swal.fire({
    ...
    window.transactionCompleteTaskById = (id) => {
        const task = tasksCache.find(t => t.id == id);
        if (task) transactionCompleteTask(task);
    };

    window.revertToCompletedTaskById = (id) => {
        const task = tasksCache.find(t => t.id == id);
        if (task) revertToCompletedTask(task);
    };
});
</script>
